@props([
    ...
    'buttonLink' => null,
    'sectionId' => 'featured-cards',
])

@php
    // Default fallback data if no solutions provided (for backward compatibility)
    $featuredSolutions = !empty($solutions) ? $solutions : [];
@endphp

@if(!empty($featuredSolutions))
<section class="relative w-full py-12 bg-white featured-cards-section" id="{{ $sectionId }}">
    <div class="mx-auto max-w-[1280px] px-4 md:px-8 w-full">
        
        @if($title || $description)
        <!-- Section Header -->
        <div class="text-center mb-12 md:mb-16 featured-cards-header">
            @if($title)
            <h2 class="text-3xl md:text-4xl lg:text-5xl font-bold mb-4 featured-cards-title">
                {{ $title }}
            </h2>
            @endif
            @if($description)
            <p class="text-base md:text-lg text-gray-600 max-w-6xl mx-auto leading-relaxed featured-cards-description">
                {{ $description }}
            </p>
            @endif
        </div>
        ...
        <div class="mb-8">
            @foreach($featuredSolutions as $index => $solution)
                <div class="featured-solution-item flex flex-col md:flex-row {{ $index % 2 === 1 ? 'md:flex-row-reverse' : '' }} gap-6 md:gap-8 items-stretch p-6 md:p-8 overflow-hidden">
                    ...
                    <div class="w-full md:w-1/3 flex flex-col justify-center featured-content">
                        ...
                        @if(isset($solution['link']) && $solution['link'])
                            <a href="{{ $solution['link'] }}" class="max-w-max border-2 border-[#0D0DE0] text-[#0D0DE0] px-6 py-2.5 rounded-full hover:bg-[#0D0DE0] hover:text-white transition-colors duration-300 text-sm md:text-base featured-button inline-block text-center">
                                {{ $solution['button_text'] ?? $buttonText }}
                            </a>
                        @elseif($buttonLink)
                            <a href="{{ $buttonLink }}" class="max-w-max border-2 border-[#0D0DE0] text-[#0D0DE0] px-6 py-2.5 rounded-full hover:bg-[#0D0DE0] hover:text-white transition-colors duration-300 text-sm md:text-base featured-button inline-block text-center">
                                {{ $buttonText }}
                            </a>
                        @else
                    ...
                    <div class="w-full md:w-2/3 featured-image-wrapper flex items-stretch">
                        <div class="relative overflow-hidden w-full featured-image-container rounded-md">
                            <img 
                                src="{{ $solution['image'] ?? asset('images/default.png') }}" 
                                alt="{{ $solution['title'] ?? 'Featured' }}"
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const solutionsSection = document.querySelector('.featured-cards-section');
        const featuredItems = document.querySelectorAll('.featured-solution-item');
        const solutionsHeader = document.querySelector('.featured-cards-header');
        const solutionsTitle = document.querySelector('.featured-cards-title');
        const solutionsDescription = document.querySelector('.featured-cards-description');
